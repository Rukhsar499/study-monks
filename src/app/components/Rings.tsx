...
export default function Rings() {
  ...
  return (
    <div
      className="relative flex items-center justify-center rounded-full w-[90vw] h-[90vw] max-w-[500px] max-h-[500px] md:w-[500px] md:h-[500px]"
    ...
    >
      {rings.map((ring) => {
        ...
        return (
          <div
            ...
            className={`
              absolute rounded-full border border-white cursor-pointer
              transition-transform duration-300 ease-in-out
              ${ring.type === "outer" ? "animate-[outerPulse_3s_ease-in-out_infinite]" : ""}
              hover:scale-105
            `}
            style={{
              width: ring.size,
              ...
              background:
                ring.type === "outer"
                  ? "rgba(215,236,229,0.6)"
                  : isActive
                  ? colors[ring.type as RingType]
                  : "#D7ECE5",
              filter: ring.type === "outer" ? "blur(15px)" : undefined,
              boxShadow: ring.type === "outer" ? "inset 0 0 30px rgba(0,0,0,0.1)" : undefined,
            }}
          />
        );
      })}
      ...
      >
        ...
        {rings.map((ring) => (
          <text
            key={`text-${ring.type}`}
            className="font-poppins fill-black"
            style={{
